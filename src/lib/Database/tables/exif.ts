...
import { exiftoolGetter, IExifData, IExifPartialData, TAvailableFields } from '../../../utils/exiftool.js';
...
function buildExifItem(exif: IExifData<'FileName'>): IMediaExifItem {
...
export async function buildExif(name: string, path: string, options: IMediaExifOptions = {}): Promise<IMediaExifList> {
    ...
    const result = _.keyBy(data.map(buildExifItem), '_raw.FileName');
